...
KERNEL(reorder_weights_int4)(const __global INPUT0_TYPE* input, __global OUTPUT_TYPE* output) {
    ...
    // Extract 4-bit values from the input buffer. gws0 covers the osv, so a feature
    // past OFM has no input row. It is padding in the output and must read as zero.
    INPUT0_TYPE in0 = 0;
    if (o0 < INPUT0_OFM_NUM) {
        const uint input0_offset = GET_FILTER_INDEX(INPUT0, 0, o0, i, 0, 0);
        in0 = (input[input0_offset / 2] >> (input0_offset % 2)*4) & 0x0F;
    ...
    // Extract 8-bit packed value from the input buffer. gws0 is OFM aligned up to the
    // osv, so the last work-items have no input row. They still own an output slot,
    // which is padding and must read as zero.
    INPUT0_TYPE packed_out_channels = 0;
    if (o < INPUT0_OFM_NUM) {
        const uint input0_offset = GET_FILTER_INDEX(INPUT0, 0, o, i, 0, 0);
        packed_out_channels = input[input0_offset / 2] & 0xFF;
