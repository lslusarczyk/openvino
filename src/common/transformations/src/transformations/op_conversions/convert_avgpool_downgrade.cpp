...
ConvertAvgPool14ToAvgPool1::ConvertAvgPool14ToAvgPool1() {
    ...
    const matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](pattern::Matcher& m) {
        ...
        if (!exclude_pad && rounding_type_v14 == ov::op::RoundingType::CEIL_TORCH) {
            ...
            const auto pads_begin_v1 = node_registry.make<v0::Concat>(OutputVector{pads_remaining, pads_begin_node}, 0);
            const auto pads_end_v1 = node_registry.make<v0::Concat>(OutputVector{pads_remaining, pads_end_node}, 0);
            const auto pad_node =
